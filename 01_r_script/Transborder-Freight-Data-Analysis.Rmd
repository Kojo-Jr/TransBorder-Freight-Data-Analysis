...
```{r}
# This code is using column-specific conditions or individual imputation conditions. This means that each column is evaluated independently for missing values, rather than requiring a combined condition (i.e. both columns missing) for imputation.
...
# For MEXSTATE: Fill NA values where COUNTRY equals 2010
mex_mask <- is.na(data$MEXSTATE) & (data$COUNTRY == 2010)
data$MEXSTATE[mex_mask] <- getmode(data$MEXSTATE)

# For CANPROV: Fill NA values where COUNTRY equals 1220
can_mask <- is.na(data$CANPROV) & (data$COUNTRY == 1220)
data$CANPROV[can_mask] <- getmode(data$CANPROV)
```
