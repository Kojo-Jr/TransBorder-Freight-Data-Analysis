...
# Since they are still missing NA's  this code uses column-specific conditions or individual imputation conditions. This means that each column is evaluated independently for missing values, rather than requiring a combined condition (i.e. both columns missing) for imputation.
...
# For MEXSTATE: Fill all NA values
mex_mask <- is.na(data$MEXSTATE)
data$MEXSTATE[mex_mask] <- getmode(data$MEXSTATE)

# For CANPROV: Fill all NA values
can_mask <- is.na(data$CANPROV)
data$CANPROV[can_mask] <- getmode(data$CANPROV)
...
transportation_mode <- ggplot(data, aes(
  ...
  scale_y_continuous(labels = label_number(scale = 1, suffix = "bn")) +  # Display values in trillions
  labs(
...
for(year in years) {
  ...
  # Summarise the total freight weight by state 
  ...
  plot <- ggplot(state_freight_summary, aes(x = reorder(USASTATE, Total_ShipWt), y = Total_ShipWt / 1e3)) +
    geom_col(fill = "steelblue") +   # Bar colour
    coord_flip() +                   # Flip the bars horizontally
    scale_y_continuous(labels = label_comma()) +  # Format the numeric axis (now horizontal) with commas
    labs(
    ...
    theme(
      ...
      plot.margin = margin(10, 50, 10, 10)    # Increase plot margins
    )
...
ggplot(yearly_trends, aes(x = as.factor(YEAR), y = Total_Value / 1e12)) +
